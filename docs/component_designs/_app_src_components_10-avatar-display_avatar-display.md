# Detalle de Diseño y Textos: /src/components/10-avatar-display/avatar-display.tsx
...
- **<div>**: {/_ Backpacker _/} {selectedBackpacker && selectedBackpacker.name !== 'Ninguno' && ( <div className="absolute text-5xl -right-8 top-1/3 z-0 motion-safe:animate-float">{selectedBackpacker.icon}</div> )} {/_ Body _/} {/_ Head _/} {/_ Head Item _/} {selectedHead && selectedHead.name !== 'Ninguno' && ( <div className="absolute text-6xl left-1/2 -translate-x-1/2 -top-4 z-20">{selectedHead.icon}</div> )} {/_ Torso Item _/} {selectedTorso && selectedTorso.name !== 'Ninguno' && ( <div className="absolute text-6xl left-1/2 -translate-x-1/2 top-20 z-20">{selectedTorso.icon}</div> )}
...
export function AvatarDisplay({ config, className }: AvatarDisplayProps) {
  ...
  const selectedBackpacker = backpackerOptions.find(
    (b) => b.name === config.backpacker,
  );
  ...
  return (
    <div className={cn("relative w-48 h-64", className)}>
      ...
      {selectedBackpacker && selectedBackpacker.name !== "Ninguno" && (
        <div className="absolute text-5xl -right-8 top-1/3 z-0 motion-safe:animate-float">
          {selectedBackpacker.icon}
        </div>
      )}
      {/* Body */}
      <div
        className={cn(
          "absolute w-32 h-40 left-1/2 -translate-x-1/2 top-16 rounded-t-3xl rounded-b-lg",
          selectedFur?.color,
        )}
      >
        {selectedFur?.name === "Cósmico" && (
      ...
      <div
        className={cn(
          "absolute w-36 h-36 left-1/2 -translate-x-1/2 top-0 rounded-full",
          selectedFur?.color,
        )}
      >
        {selectedFur?.name === "Cósmico" && (
          <div className="absolute inset-0 rounded-full stars opacity-50" />
        )}
      ...
      {selectedHead && selectedHead.name !== "Ninguno" && (
        <div className="absolute text-6xl left-1/2 -translate-x-1/2 -top-4 z-20">
          {selectedHead.icon}
        </div>
      )}
      {/* Torso Item */}
      {selectedTorso && selectedTorso.name !== "Ninguno" && (
        <div className="absolute text-6xl left-1/2 -translate-x-1/2 top-20 z-20">
          {selectedTorso.icon}
        </div>
      )}
    </div>
