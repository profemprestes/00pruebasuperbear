...
- `relative bg-white border-6 sm:border-8 border-teddy-brown rounded-2xl p-3 sm:p-6 shadow-3d w-full max-w-3xl text-center motion-safe:animate-in fade-in-0 zoom-in-95 slide-in-from-bottom-8 duration-500`
- `font-milky text-2xl sm:text-3xl md:text-4xl text-teddy-brown mb-3 sm:mb-4`
- `relative bg-gray-200 h-72 sm:h-80 md:h-96 w-full rounded-lg border-4 border-gray-400 overflow-hidden`
- `p-3 sm:p-4 h-full flex flex-col items-center`
- `font-body font-bold text-sm sm:text-lg mb-1 sm:mb-2 text-center`
- `relative w-full h-full`
- `absolute text-3xl sm:text-4xl motion-safe:animate-float`
- `font-body font-bold text-xs sm:text-sm md:text-lg mb-2 text-center leading-tight`
...
- `absolute text-4xl sm:text-5xl motion-safe:animate-float hover:bg-transparent`
- `absolute text-4xl sm:text-5xl motion-safe:animate-sway`
- `font-body font-bold text-sm sm:text-lg mb-2 text-center`
- `flex justify-around w-full mt-4 sm:mt-8`
- `w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-stone-700 rounded-full flex items-center justify-center`
- `text-3xl sm:text-4xl md:text-5xl motion-safe:animate-in fade-in-0 zoom-in-75 hover:bg-transparent`
- `w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full bg-gradient-to-tr from-red-500 via-yellow-500 to-blue-500 font-milky text-base sm:text-lg md:text-2xl shadow-lg motion-safe:animate-float`
- `w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full bg-gradient-to-tr from-red-500 via-yellow-500 to-blue-500 motion-safe:animate-roulette flex items-center justify-center`
...
- **<div>**: {renderMinigame()}
- **<p>**: ¡Recoge 5 monedas del Arcade!
- **<div>**: {coins.map(coin => !coin.collected && ( <button key={coin.id} onClick={() => handleCollect(coin.id)} aria-label={`Recoger moneda ${coin.id + 1} del Arcade`} className="absolute text-3xl sm:text-4xl motion-safe:animate-float" style={{ top: coin.top, left: coin.left }}> 🪙 </button> ))}
- **<button>**: 🪙
...
- **<div>**: 🟣
- **<div>**: 🟣
- **<p>**: ¡Despierta a los 3 topos dormidos! ({whacked} /3)
- **<div>**: {moles.map(mole => ( <div key={mole.id} className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-stone-700 rounded-full flex items-center justify-center"> {mole.active && ( <Button onClick={handleWhack} aria-label="Golpear topo" variant="ghost" size="icon" className="text-3xl sm:text-4xl md:text-5xl motion-safe:animate-in fade-in-0 zoom-in-75 hover:bg-transparent" > 🔥 </Button> )} </div> ))}
- **<div>**: {mole.active && ( <Button onClick={handleWhack} aria-label="Golpear topo" variant="ghost" size="icon" className="text-3xl sm:text-4xl md:text-5xl motion-safe:animate-in fade-in-0 zoom-in-75 hover:bg-transparent" > 🔥 </Button> )}
- **<Button>**: 🔥
- **<div>**: {!spinning ? ( <Button onClick={handleSpin} aria-label="Girar la ruleta de Capitalos para obtener recompensa" variant="3d-gold" size="game-lg" className="w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full bg-gradient-to-tr from-red-500 via-yellow-500 to-blue-500 font-milky text-base sm:text-lg md:text-2xl shadow-lg motion-safe:animate-float" > GIRAR </Button> ) : ( <div className="w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full bg-gradient-to-tr from-red-500 via-yellow-500 to-blue-500 motion-safe:animate-roulette flex items-center justify-center"> <div className="w-[calc(100%-8px)] h-[calc(100%-8px)] rounded-full bg-white flex items-center justify-center font-milky text-teddy-brown text-xs sm:text-sm md:text-lg text-center p-2"> ¡Premio Legendario! </div> </div> )}
- **<p>**: ¡Gira la ruleta de Capitalos para tu recompensa final!
- **<Button>**: GIRAR
