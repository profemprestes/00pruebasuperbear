  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center w-full max-w-4xl mx-auto animate-in fade-in-0 duration-500">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4 md:gap-6 mb-12">
          {inventoryItems.map((item, index) => (
            <div key={index} className="bg-white/10 border-2 border-golden-coin rounded-full shadow-lg flex items-center justify-center py-2 px-3 sm:px-4">
              <span className="text-xl sm:text-2xl mr-2">{item.icon}</span>
              <p className="font-body text-white font-bold text-sm sm:text-md">{item.text}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-12 p-4">
            <div className="group bg-white p-4 pb-12 shadow-lg border-4 border-gray-300 relative transform md:-rotate-6 transition-all duration-300 ease-in-out hover:scale-105 md:hover:rotate-0 hover:z-10 w-full max-w-[250px] sm:max-w-[300px]">
                <Image src="/facu.jpg" alt="Facu en Taekwondo" width={300} height={300} className="w-full object-cover" />
            </div>
            <div className="group bg-white p-4 pb-12 shadow-lg border-4 border-gray-300 relative transform md:rotate-6 transition-all duration-300 ease-in-out hover:scale-105 md:hover:rotate-0 hover:z-10 w-full max-w-[250px] sm:max-w-[300px]">
                <Image src="/facu2.jpeg" alt="Retrato de Facu" width={300} height={300} className="w-full object-cover" />
            </div>
        </div>
